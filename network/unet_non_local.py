...
"""Implementation of the non-local Unet in torch,
Author: zhangfan
...
from network.blocks.attention import MultiHeadAttention3d
...
parser.add_argument('--num_filters', type=int, default=8, help='number of filters for initial_conv')
conf = parser.parse_args()


# basic modules
def bn_relu(filters):
    sequence = [nn.GroupNorm(num_groups=filters // 4, num_channels=filters), nn.ReLU6(inplace=True)]
...
class Network(nn.Module):
    def __init__(self, conf):
        ...
        # Output Block
        self.output_block = nn.Sequential(*[nn.GroupNorm(num_groups=num_filters // 4, num_channels=num_filters),
...
class AttDecodeingBlockLayer(nn.Module):
    """attention decoding block layer"""
